...
export default function ApprovedEmployeeRequisitions() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await getManpowerRequisitionsService(user.token, 'APPROVED');
        ...
        if (response.data?.statusCode === 200) {
          ...
          setNotFoundText('No approved requisitions are available!');
        } else {
