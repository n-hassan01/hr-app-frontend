...
export default function PendingEmployeeRequisitions() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await getManpowerRequisitionsService(user.token, 'PENDING');
        ...
        if (response.data?.statusCode === 200) {
          ...
          setNotFoundText('No pending requisitions are available!');
        } else {
